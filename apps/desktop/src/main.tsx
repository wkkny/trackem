...
import { LuActivity, LuBlocks, LuFileText, LuLayoutDashboard, LuMoon, LuRefreshCw, LuSettings, LuSun, LuX } from '@trackem/ui/icons';
import { AiOutlineOpenAI } from '@trackem/ui/icons';
import { SiClaude } from '@trackem/ui/icons';
import './index.css';
...
import { formatDuration, percentLeft } from '@trackem/core/usage';
import { cn } from '@trackem/ui/utils';
import { Button } from '@trackem/ui/components/button';
import { Empty, EmptyContent, EmptyDescription, EmptyHeader, EmptyMedia, EmptyTitle } from '@trackem/ui/components/empty';
import { Progress } from '@trackem/ui/components/progress';
import { Separator } from '@trackem/ui/components/separator';
import { Spinner } from '@trackem/ui/components/spinner';
import { SettingsPage } from '@/components/settings';
import { ResearchPage } from '@/components/research';

const IS_MAC = /Mac/.test(navigator.platform);
const IS_POPOVER = new URLSearchParams(window.location.search).get('view') === 'popover';
...
function App() {
  ...
  const [data, setData] = useState<UsageSnapshotPayload>({ codex: [], claude: [], diagnostics: [], lastCheckedAt: null, appVersion: '' });
  ...
  useEffect(() => {
    ...
    const unsubscribeNavigation = api.onNavigate(setActive);
    void api.getPage().then(setActive).catch(() => setError('Could not load dashboard navigation.'));
  ...
  if (IS_POPOVER) return (
    <main className="flex h-screen flex-col bg-background text-foreground" aria-label="Trackem tray usage">
      <header className="flex shrink-0 items-center justify-between border-b p-4"><h1 className="font-heading text-lg font-bold">Trackem{data.appVersion ? <span className="ml-2 align-middle text-xs font-normal text-muted-foreground">v{data.appVersion}</span> : null}</h1><div className="flex gap-2">{refreshButton}<Button variant="ghost" size="icon" onClick={() => window.trackem?.minimize()} aria-label="Hide to tray"><LuX /></Button></div></header>
      ...
      <footer className="flex shrink-0 items-center justify-between gap-2 border-t p-4"><p className="text-xs text-muted-foreground">Credentials stay local{data.appVersion ? ` · v${data.appVersion}` : ''}</p><Button variant="outline" onClick={() => window.trackem?.openDashboard()}>Open dashboard</Button></footer>
    </main>
  );
  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-background text-foreground">
      <div className="grid h-[52px] shrink-0 grid-cols-[88px_1fr_88px] items-center border-b bg-card select-none [-webkit-app-region:drag]">
        <div />        <div className="flex items-center gap-2 justify-self-center font-heading text-base font-bold"><TrackemMark className="size-5" />trackem{data.appVersion ? <span className="text-xs font-normal text-muted-foreground">v{data.appVersion}</span> : null}</div>
