...
3. Open the dashboard, hide it, and repeat 20 times. Check keyboard navigation, focus, text clipping, light/dark themes, and taskbar auto-hide.
...
- Disconnect the network. Requests should time out, never overlap, and back off up to 30 minutes if every enabled account fails. Manual refresh has a 30-second cooldown. Resume triggers a fresh check.
